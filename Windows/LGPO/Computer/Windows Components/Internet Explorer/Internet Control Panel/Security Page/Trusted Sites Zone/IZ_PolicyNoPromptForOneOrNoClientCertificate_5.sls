...
#
####### LGPO DETAILS ########
# LOCATION: \Computer\Windows Components\Internet Explorer\Internet Control Panel\Security Page\Trusted Sites Zone\IZ_PolicyNoPromptForOneOrNoClientCertificate_5.sls
#
...
# 
# If you do not configure this policy setting, Internet Explorer does not prompt users with a "Client Authentication" message when they connect to a Web site that has no certificate or only one certificate.
Do not prompt for client certificate selection when no certificates or only one certificate exists.:
  lgpo.set:
  - name: IZ_PolicyNoPromptForOneOrNoClientCertificate_5
  - setting:
